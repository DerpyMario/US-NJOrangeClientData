...
def procCrawl(dst, src, ldr = None, ls = None):
    ...
    os.makedirs(dst, exist_ok = True)
    for t in [
        _run(CddLdr("GameData.bin", DOMAIN + "OrangeData.bin").crawl, dst, src, DOMAIN + "325/Designs/", gHdr),
        _run(CddLdr("TextData.bin", DOMAIN + "OrangeTextData.bin").crawl, dst, src, DOMAIN + "325/Designs/", gHdr),
        _run(CddLdr("LocalData.bin", DOMAIN + "LocalTextData.bin").crawl, dst, src, DOMAIN, gHdr),
        ...
        _run(JsonLdr("0.0.0.bin").crawl, dst, src, DOMAIN, gHdr)
    ]: t.join()
